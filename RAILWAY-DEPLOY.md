...
- **GitHub repo:** `vin371/Japanese-Learning-Chat-Website--EXE201` (branch `main`)
- **Root directory:** để **trống** (repo gốc — có `Dockerfile`, `railway.toml`). Nếu đặt `backend` → build lỗi → **502**
- **Builder:** Dockerfile (`railway.toml` đã cấu hình)
- **Networking:** domain public → cổng do Railway gán (`PORT`). **Xóa** biến `API_HOST_PORT` nếu có
- Sau mỗi push GitHub: **Deployments** → deployment mới **Active** (commit `631a933` trở lên)
...
|-------------|-------------|
| **502 Application failed to respond** | Xem **Deployments → View logs**: crash DB/JWT, Root directory sai, chưa **Deploy** Variables, hoặc build Docker fail |
