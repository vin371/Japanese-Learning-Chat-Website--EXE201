...
/// <summary>Import khóa N4 từ 3 file DOCX (từ vựng, ngữ pháp, hán tự) — phân bài theo chủ đề.</summary>
public static class N4DocxCourseImporter
{
    private const int N4LevelId = 2;
    ...
    public static async Task<int> RunAsync(ApplicationDbContext db, string importDir, bool dryRun = false)
    {
        ...
        if (vocabPath == null || grammarPath == null || kanjiPath == null)
            throw new FileNotFoundException(
                $"Thiếu file DOCX trong {importDir}. Cần: Từ vựng, Ngữ pháp, Hán tự N4.");

        Console.WriteLine($"[import-n4] Vocab:   {Path.GetFileName(vocabPath)}");
        Console.WriteLine($"[import-n4] Grammar: {Path.GetFileName(grammarPath)}");
        Console.WriteLine($"[import-n4] Kanji:   {Path.GetFileName(kanjiPath)}");
        ...
        Console.WriteLine(
            $"[import-n4] N4 — từ vựng: {vocabLessons.Count} bài, ngữ pháp: {grammarLessons.Count} bài, kanji: {kanjiLessons.Count} bài");
        ...
        var n4CategoryIds = await db.LessonCategories
            .Where(c => c.LevelId == N4LevelId)
            .Select(c => c.Id)
            .ToListAsync();

        var lessonIds = await db.Lessons
            .Where(l => n4CategoryIds.Contains(l.CategoryId))
            .Select(l => l.Id)
            .ToListAsync();
        ...
        if (lessonIds.Count > 0)
        {
            ...
            Console.WriteLine($"[import-n4] Đã xóa {lessons.Count} bài N4 cũ.");
        }

        var now = DateTime.UtcNow;
        var catVocab = await db.LessonCategories.FirstAsync(c => c.LevelId == N4LevelId && c.Type == "vocabulary");
        var catGrammar = await db.LessonCategories.FirstAsync(c => c.LevelId == N4LevelId && c.Type == "grammar");
        var catKanji = await GetOrCreateKanjiCategoryAsync(db, now);
        ...
        Console.WriteLine($"[import-n4] Hoàn tất — {inserted} bài mới.");
        return inserted;
    }

    private static async Task<LessonCategory> GetOrCreateKanjiCategoryAsync(ApplicationDbContext db, DateTime now)
    {
        var existing = await db.LessonCategories
            .FirstOrDefaultAsync(c => c.LevelId == N4LevelId && c.Type == "kanji");
        ...
        await db.SaveChangesAsync();
        Console.WriteLine($"[import-n4] Đã tạo danh mục Kanji N4 (id={cat.Id}).");
        return cat;
    }
        ...
        ApplicationDbContext db, int categoryId, List<ParsedLesson> lessons, DateTime now)
    {
        ...
        {
            var slug = Slugify($"n4-vocab-bai-{pl.Number}-{pl.Title}");
        ...
        ApplicationDbContext db, int categoryId, List<ParsedLesson> lessons, DateTime now)
    {
        ...
        foreach (var pl in lessons.OrderBy(l => l.Number))
        {
            var slug = Slugify($"n4-grammar-bai-{pl.Number}-{pl.Title}");
            var lesson = new Lesson
            {
                CategoryId = categoryId,
                Title = pl.Title,
        ...
        ApplicationDbContext db, int categoryId, List<ParsedLesson> lessons, DateTime now)
    {
        ...
        foreach (var pl in lessons.OrderBy(l => l.Number))
        {
            var slug = Slugify($"n4-kanji-bai-{pl.Number}-{pl.Title}");
            var lesson = new Lesson
            {
                CategoryId = categoryId,
                Title = pl.Title,
    ...
    private static string BuildVocabIntroHtml(ParsedLesson pl) =>
        $"<p class=\"learn-intro\">Chủ đề <strong>{Esc(pl.Title)}</strong> — {pl.VocabRows.Count} từ vựng JLPT N4.</p>";

    private static string BuildGrammarIntroHtml(ParsedLesson pl) =>
        $"<p class=\"learn-intro\">Ngữ pháp N4: <strong>{Esc(pl.Title)}</strong> — {pl.GrammarRows.Count} mẫu câu.</p>";

    private static string BuildKanjiIntroHtml(ParsedLesson pl) =>
        $"<p class=\"learn-intro\">Hán tự N4: <strong>{Esc(pl.Title)}</strong> — {pl.KanjiRows.Count} chữ Kanji.</p>";
    ...
    /// <summary>File DOCX chỉ chứa N4 — dừng khi gặp mục N3.</summary>
    private static List<ParsedLesson> ParseDocx(string path, DocxKind kind)
    {
        ...
        foreach (var el in body.ChildElements)
        {
            ...
            if (el is Paragraph p)
            {
                ...
                if (IsLevelStop(text, stopAtLevel: 3))
                {
                    if (lessons.Count > 0)
                    {
                        stopFile = true;
                ...
                if (!inN4 && !LessonHeaderRx.IsMatch(text)) continue;
            ...
            else if (el is Table tbl && current != null && inN4)
            {
    ...
    {
        if (stopAtLevel != 3) return false;
        var t = text.ToUpperInvariant();
        var compact = Regex.Replace(t, @"\s+", "");
        if (compact.Contains("NGUPHAPN3") || compact.Contains("TUVUNGN3") || compact.Contains("HANTUN3") || compact.Contains("KANJIN3"))
            return true;
        if (Regex.IsMatch(t, @"\bN3\b") &&
