...
/// <summary>Import khóa N3 từ 3 file DOCX (từ vựng, ngữ pháp, hán tự) — phân bài theo chủ đề.</summary>
public static class N3DocxCourseImporter
{
    private const int N3LevelId = 3;
    ...
    {
        var vocabPath = FindDocx(importDir, "vựng", "vung", "Vựng");
        ...
        if (vocabPath == null || grammarPath == null || kanjiPath == null)
            throw new FileNotFoundException(
                $"Thiếu file DOCX trong {importDir}. Cần: Từ vựng, Ngữ pháp, Hán tự N3.");

        Console.WriteLine($"[import-n3] Vocab:   {Path.GetFileName(vocabPath)}");
        Console.WriteLine($"[import-n3] Grammar: {Path.GetFileName(grammarPath)}");
        Console.WriteLine($"[import-n3] Kanji:   {Path.GetFileName(kanjiPath)}");
        ...
        Console.WriteLine(
            $"[import-n3] N3 — từ vựng: {vocabLessons.Count} bài, ngữ pháp: {grammarLessons.Count} bài, kanji: {kanjiLessons.Count} bài");
        ...
        var n3CategoryIds = await db.LessonCategories
            .Where(c => c.LevelId == N3LevelId)
            .Select(c => c.Id)
            .ToListAsync();

        var lessonIds = await db.Lessons
            .Where(l => n3CategoryIds.Contains(l.CategoryId))
            .Select(l => l.Id)
            .ToListAsync();
        ...
        if (lessonIds.Count > 0)
        {
            ...
            Console.WriteLine($"[import-n3] Đã xóa {lessons.Count} bài N3 cũ.");
        }

        var now = DateTime.UtcNow;
        var catVocab = await GetOrCreateCategoryAsync(db, "vocabulary", "Từ vựng N3", "vocab-n3", 1, now);
        var catGrammar = await GetOrCreateCategoryAsync(db, "grammar", "Ngữ pháp N3", "grammar-n3", 2, now);
        var catKanji = await GetOrCreateCategoryAsync(db, "kanji", "Kanji N3", "kanji-n3", 5, now);
        ...
        Console.WriteLine($"[import-n3] Hoàn tất — {inserted} bài mới.");
        return inserted;
    }

    private static async Task<LessonCategory> GetOrCreateCategoryAsync(
        ApplicationDbContext db, string type, string name, string slug, int sortOrder, DateTime now)
    {
        var existing = await db.LessonCategories
            .FirstOrDefaultAsync(c => c.LevelId == N3LevelId && c.Type == type);
        if (existing != null) return existing;

        var cat = new LessonCategory
        {
            LevelId = N3LevelId,
            Name = name,
            Slug = slug,
        ...
        db.LessonCategories.Add(cat);
        await db.SaveChangesAsync();
        Console.WriteLine($"[import-n3] Đã tạo danh mục {name} (id={cat.Id}).");
        return cat;
    }
        ...
        ApplicationDbContext db, int categoryId, List<ParsedLesson> lessons, DateTime now)
    {
        ...
        {
            var slug = Slugify($"n3-vocab-bai-{pl.Number}-{pl.Title}");
        ...
        ApplicationDbContext db, int categoryId, List<ParsedLesson> lessons, DateTime now)
    {
        ...
        foreach (var pl in lessons.OrderBy(l => l.Number))
        {
            var slug = Slugify($"n3-grammar-bai-{pl.Number}-{pl.Title}");
            var lesson = new Lesson
            {
                CategoryId = categoryId,
                Title = pl.Title,
        ...
        ApplicationDbContext db, int categoryId, List<ParsedLesson> lessons, DateTime now)
    {
        ...
        foreach (var pl in lessons.OrderBy(l => l.Number))
        {
            var slug = Slugify($"n3-kanji-bai-{pl.Number}-{pl.Title}");
            var lesson = new Lesson
            {
                CategoryId = categoryId,
                Title = pl.Title,
    ...
    private static string BuildVocabIntroHtml(ParsedLesson pl) =>
        $"<p class=\"learn-intro\">Chủ đề <strong>{Esc(pl.Title)}</strong> — {pl.VocabRows.Count} từ vựng JLPT N3.</p>";

    private static string BuildGrammarIntroHtml(ParsedLesson pl) =>
        $"<p class=\"learn-intro\">Ngữ pháp N3: <strong>{Esc(pl.Title)}</strong> — {pl.GrammarRows.Count} mẫu câu.</p>";

    private static string BuildKanjiIntroHtml(ParsedLesson pl) =>
        $"<p class=\"learn-intro\">Hán tự N3: <strong>{Esc(pl.Title)}</strong> — {pl.KanjiRows.Count} chữ Kanji.</p>";

    private static string Esc(string s) =>
        s.Replace("&", "&amp;").Replace("<", "&lt;").Replace(">", "&gt;");

    private static List<ParsedLesson> ParseVocabDocx(string path) =>
        ParseDocx(path, DocxKind.Vocabulary);

    private static List<ParsedLesson> ParseGrammarDocx(string path) =>
        ParseDocx(path, DocxKind.Grammar);

    private static List<ParsedLesson> ParseKanjiDocx(string path) =>
        ParseDocx(path, DocxKind.Kanji);
    ...
    /// <summary>File DOCX chỉ chứa N3 — dừng khi gặp mục N2.</summary>
    private static List<ParsedLesson> ParseDocx(string path, DocxKind kind)
    {
        ...
        foreach (var el in body.ChildElements)
        {
            ...
            if (el is Paragraph p)
            {
                ...
                if (!inN3 && !LessonHeaderRx.IsMatch(text)) continue;
            ...
            else if (el is Table tbl && current != null && inN3)
            {
    ...
    private static bool IsLevelStop(string text)
    {
        var t = text.ToUpperInvariant();
        var compact = Regex.Replace(t, @"\s+", "");
        if (compact.Contains("NGUPHAPN2") || compact.Contains("TUVUNGN2") || compact.Contains("HANTUN2") || compact.Contains("KANJIN2"))
            return true;
        if (Regex.IsMatch(t, @"\bN2\b") &&
